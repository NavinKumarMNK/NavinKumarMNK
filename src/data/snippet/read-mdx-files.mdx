...
summary: 'Most dev use MDX as their storage to store content such as post, well this site use mdx too, this snippet useful to read mdx files'
topic: 'nodejs'
...
```js
// notice I don't need to prefix the first parameter to be `/post`
// or the second paramter to be `my-first-post.mdx`
const post = await getContentBySlug('post', 'my-first-post')
```
...
const post = await getContentBySlug<Post>('post', 'my-first-post')
